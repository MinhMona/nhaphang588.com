...
namespace NHST.Controllers
{
    public class SmallPackageController
    {
        ...
        public static string UpdateCODTQ(int ID, string CODTQCYN, string CODTQVND, DateTime ModifiedDate, string ModifiedBy)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_SmallPackage.Where(x => x.ID == ID).FirstOrDefault();
    ...
    int Status, string Description, DateTime ModifiedDate, string ModifiedBy)
        ...
        public static List<tbl_SmallPackage> GetBuyBigPackageIDKhoVN(int BigPackageID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_SmallPackage> ps = new List<tbl_SmallPackage>();
                ps = dbe.tbl_SmallPackage.Where(p => p.BigPackageID == BigPackageID && p.Status != 4).OrderByDescending(p => p.ID).ToList();
                return ps;
            }
        }
        ...
        public static List<tbl_SmallPackage> GetListByOrderTransactionCodeKhoChina(string OrderTransactionCode)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_SmallPackage> lmls = new List<tbl_SmallPackage>();
                lmls = dbe.tbl_SmallPackage.Where(ad => ad.OrderTransactionCode == OrderTransactionCode && (ad.Status != 4) && (ad.Status != 3)).ToList();
                return lmls;
            }
        }
