...
namespace NHST.Controllers
{
    public class NotificationsController
    {
        ...
        public static List<tbl_Notification> GetAllTOP(int UID)
        {
            using (var dbe = new NHSTEntities())
            {
                var n = dbe.tbl_Notification.Where(x => x.ReceivedID == UID && x.Status == 1).OrderByDescending(x => x.ID).Take(100).ToList();
                if (n != null)
                    return n;
                return null;
            }
