...
namespace NHST.Controllers
{
    public class BigPackageController
    {
        ...
        public static tbl_BigPackage GetByPackageCodeStatus(string PackageCode)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_BigPackage a = dbe.tbl_BigPackage.Where(ad => ad.PackageCode == PackageCode && ad.Status == 1).FirstOrDefault();
                if (a != null)
                {
                    return a;
                }
