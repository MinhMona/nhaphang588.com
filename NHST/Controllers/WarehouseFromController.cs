...
namespace NHST.Controllers
{
    public class WarehouseFromController
    {
        ...
        public static List<tbl_WarehouseFrom> GetAllWithIsHidden(bool IsHidden)
        {
            ...
            {
                List<tbl_WarehouseFrom> cs = new List<tbl_WarehouseFrom>();
                cs = dbe.tbl_WarehouseFrom.Where(c => c.IsHidden == IsHidden).ToList();
                return cs;
            }
        }
        public static List<tbl_WarehouseFrom> GetAllWithIsHiddenOrder(bool IsHidden)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_WarehouseFrom> cs = new List<tbl_WarehouseFrom>();
                cs = dbe.tbl_WarehouseFrom.Where(c => c.IsHidden == IsHidden && c.ID != 3).ToList();
                return cs;
            }
        }        
