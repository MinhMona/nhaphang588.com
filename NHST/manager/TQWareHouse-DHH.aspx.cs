        ...
        public static string GetCode(string barcode)
        {
            ...
            if (HttpContext.Current.Session["userLoginSystem"] != null)
            {
                ...
                if (user != null)
                {
                    ...
                    {
                        var packages = SmallPackageController.GetListByOrderTransactionCodeKhoChina(barcode.Trim());
                        if (packages.Count > 0)
                        {
                            ...
                            pa.PackageAllType = 0;
                            pa.PackageGetCount = packages.Count; List<OrderGet> og = new List<OrderGet>();
                        ...
                        {
                            //Thêm mã vận đơn vào kiện trôi nổi
                            int MainOrderID = 0;
                            string temp = "";
                            ...
                            if (setNoti != null)
                            {
                                ...
                                if (acc != null)
                                {
                                    if (setNoti.IsSentNotiUser == true)
                                    {
                                        ...
                                        {
                                            NotificationsController.Inser(acc.ID, acc.Username, mainorder.ID, "Hàng của đơn hàng " + mainorder.ID + " Hàng về kho TQ.",
                                            1, currentDate, username_current, true);
                                        }
                                        else
                                        {
                                            NotificationsController.Inser(acc.ID, acc.Username, mainorder.ID, "Hàng của đơn hàng TMĐT " + mainorder.ID + " Hàng về kho TQ.",
                                    ...
                                    //    try
                                    //    {
                                    //        PJUtils.SendMailGmail("monamedia", "uvnajxzzurygsduf", acc.Email, "Thông báo tại NHẬP HÀNG 558.", "Hàng của đơn hàng " + mainorder.ID + " Hàng về kho TQ.", "");
                                    //    }
                                    //    catch { }
                                    //}
                                }
